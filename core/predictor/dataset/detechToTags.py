...
def detech2tags(input_path, output_path):
    ...
    filesp = [f.replace('FALLO', '').replace('FALLA', '').split('detechPhoto') for f in files]
    ...
    files_names = sorted([[int(f.replace('FALLO', '').replace('FALLA', '').split('detechPhoto')[0]), f] for f in files], key=lambda r:r[0])
